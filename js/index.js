			...
			projects:[
				{
					...
					project_discription:[
						...
						{
							'个人行动':'根据207+国内外流行病学研究文献，汇总 了4大类非药物干预方案，设计了个性化自适应推荐算法。通过竞品分析，设计 了MVP原型图。用了一个月自学了前端开发，用Hbuilder X完成开发，并用 unicloud云开发度过了公司没有后端工程师的局面。'
						},
						{
							'项目结果':'小程序成功上线，可以完成 toB到toC的业务;推荐算法成功申请专利CN114038534A'
						}
					]
				},
				{
					...
					project_discription:[
						...
						{
							'个人行动':'通过复盘、拆解功能模块，深入分析国内外在认知训练领域的产品，整理成分析文档 并汇报展示，向决策层给出了迭代方向的建议。通过问卷和一对一老年人深度交 流的方式，了解用户的原始需求，最后用综合运5W2H和KANO等需求分析方 法，制定相应的版本规划(包括社交圈子、自营商城、游戏化激励体系、制定线 上服务支付流程等)、产出PRD和原型图，完成前端开发。'
						},
						{
				...
				},
				{
					...
					project_discription:[
						...
						{
							'个人行动':'以技术的身份参加了项目会议，主动与第三方运营公司对接。 根据需求梳理用户路径与页面流程，完成前端代码，对关键业务流程用公众平台 工具we分析埋点，对平台埋点数据和公司数据库分析(python + sql)用户行 为、用户画像和转化漏斗，每月产出数据分析报告，辅助罗氏和第三方运营公司 制定运营策略'
						},
						{
			...
			projects:[
				{
					...
					project_discription:[
						...
						{
							'项目目标':'搭建一实验和计算机辅助设计平台。前者研究面向视障老年人群体的标识环境空间设计，需要实现信息录入、随机出题、大体量数据记录与分析等功能;后者则 是用于辅助设计师进行标识色彩组合的搭配。'
						},
						{
							'个人行动':'前期的论文研究和功能、界面流程的梳理，HBuilder X前端代码和云数据库的建立，及原始数据清 理、分析和可视化。'
						},
						{
							'项目结果':'<a href="http://signage.tju-audrc.cn/#/">交付后</a>，辅助设计部分被用于申请专利，整个研究被剑桥大 学Inclusive Design Toolkit team报道'
						}
					]
				}
			],
